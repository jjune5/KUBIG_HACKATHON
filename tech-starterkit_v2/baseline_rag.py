...
def _format_context(candidates: list[dict]) -> str:
    """v2 patch G: Spotlighting — wrap each chunk in <UNTRUSTED_CHUNK> marker.
    Tells model that chunk content is data, NOT instructions."""
    parts = []
    for rank, c in enumerate(candidates, start=1):
        tag = f' category="{c["category"]}"' if c.get("category") and c["category"] != "page" else ""
        parts.append(
            f'<UNTRUSTED_CHUNK id={rank} source="{c["source"]}" page={c["page"]}{tag}>\n'
            f"{c['text']}\n"
            f"</UNTRUSTED_CHUNK>"
        )
    return "\n\n".join(parts)
