...
# v3: mailbox name in question (e.g., "Mike Maggi" → maggi)
MAILBOX_HINT_RE = re.compile(
    r"\b(?:mailbox\s+of\s+)?([A-Z][a-z]+)\s+([A-Z][a-z]+)(?:'s\s+(?:mailbox|attachment|email|message))?",
)
...
def _search_candidates(question: str, index, limit: int = TOP_K) -> list[dict]:
    ...
    v3: corpus-wide query 검출 시 is_header chunks 강제 포함."""
    chunks = index["chunks"]
    ...
    # v3 boost: corpus-wide query 면 모든 PDF 의 header chunk 강제 포함
    if _is_corpus_wide(question):
        for cid, c in enumerate(chunks):
            if c.get("is_header") and cid not in fused:
                fused[cid] = 10.0  # 강제 상위
                payload_by_id[cid] = c
            elif c.get("is_header"):
                fused[cid] = fused.get(cid, 0) + 1.0   # 이미 있으면 boost
...
5. PRESERVE the exact spelling AND spacing of all proper nouns (company names,
   person names, addresses) exactly as they appear in the documents.
   E.g. "Watson Wyatt" (two words, NOT "WatsonWyatt"),
        "Long Island Shooting Center" (preserve all spaces),
